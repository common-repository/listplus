...
class Reports_List_Table extends \WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct(
			array(
				'singular' => __( 'Report', 'list-plus' ),     // Singular name of the listed records.
				'plural'   => __( 'Reports', 'list-plus' ),    // Plural name of the listed records.
				'ajax'     => false, // Does this table support ajax?
			)
		);
		global $wpdb;
		$this->table = Report::get_table();
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'reason'   => _x( 'Reason', 'Column label', 'list-plus' ),
	...
	protected function column_reason( $item ) {
		$html = '';
		$html .= esc_html( wp_trim_words( $item['reason'], 30 ) );
		return $html;
	}
	...
	public function prepare_items() {
		...
		$where = '';
		if ( $listing_id ) {
			Report::query()->where( 'post_id', $listing_id );
		}
		if ( $status ) {
			Report::query()->where( 'status', $status );
		}

		Report::query()->limit( $per_page );
		if ( $orderby ) {
			Report::query()->order_by( 'status', $status );
		}

		$this->items = Report::query()->find();
		$total_items = Report::query()->count();
		$total_pages = Report::query()->get_max_pages();
