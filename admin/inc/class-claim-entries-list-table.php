...
class Claim_Entries_List_Table extends \WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct(
			array(
				'singular' => __( 'Claim', 'list-plus' ),     // Singular name of the listed records.
				'plural'   => __( 'Claims', 'list-plus' ),    // Plural name of the listed records.
				'ajax'     => false, // Does this table support ajax?
			)
		);
		global $wpdb;
		$this->table = Claim::get_table();
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'email'    => _x( 'Claimer', 'Column label', 'list-plus' ),
			'content'   => _x( 'Message', 'Column label', 'list-plus' ),
	...
	protected function get_bulk_actions() {
		$actions = array(
			'approved' => _x( 'Approved claims', 'List table bulk action', 'list-plus' ),
			'pending'  => _x( 'Pending claims', 'List table bulk action', 'list-plus' ),
			'rejected' => _x( 'Rejected claims', 'List table bulk action', 'list-plus' ),
	...
	public function process_bulk_action() {
		...
		$message = '';
		$n = count( $ids );
		$all_actions = $this->get_bulk_actions();
		if ( isset( $all_actions[ $action ] ) ) {
			...
			if ( 'delete' == $action ) {
				...
				$claims = Claim::query()->where_in( 'id', $ids )->find();
	...
	public function prepare_items() {
		...
		if ( $orderby ) {
			Claim::query()->order_by( $orderby, $order );
		}

		$this->items = Claim::query()->find();
		$total_items = Claim::query()->count();
		$total_pages = Claim::query()->get_max_pages();
